...
1. 下载 `releases/skimline-0.3.2-extension.zip`。
2. 解压 ZIP，并进入其中的 `skimline-0.3.2` 目录。
...
在概览、观点和展开详情中圈选 2–200 个字，会出现“收藏 / 解释 / 复制”。收藏会立即把所选文字、来源视频和对应时间保存到本机洞见库，不触发新的模型请求；普通观点和核心洞见拥有完全相同的收藏能力。点击顶部书签入口可以搜索收藏、回到来源时间、删除并撤销删除。点击“解释”会从底部拉起完整抽屉：优先读取当前时间附近的字幕，再从整段字幕中检索相关内容，并区分通用定义与视频中的具体含义，提供可跳转的字幕依据。每次解释最多继续追问 3 轮，历史回答以折叠轮次呈现，正文区独立滚动、输入区固定在底部。收起抽屉后界面完全消失且后台任务继续；切换标签页或暂时关闭 Side Panel 不会中断生成，返回原视频可恢复结果。语言切换或明确发起另一个解释会终止不再适用的旧任务。任务快照仅保存在当前浏览器会话；字幕缺失时只给通用解释，并明确标注无法确认视频语境。
...
测试覆盖 Side Panel 声明与渲染、全局文字缩放与本地持久化、侧栏↔内容脚本消息桥、CSP 安全的 MAIN world 字幕读取、无字幕与播放器失败的区分、字幕轨道选择、json3 与官方文字记录 fallback、分块、乱序流式合并、6 秒语言确认、概览与观点并行时机、概览独立缓存和重试、失败隔离、多标签复用、最后标签页关闭取消、Service Worker 恢复、旧缓存兼容、默认值得看问题、圈选解释的字幕检索与真实时间戳约束、短追问、完整标题与 CSS 省略、折叠与跳转、双层播放跟随和关键视觉 token。`config.local.js` 仅供本地烟测，已被 Git 忽略，严禁提交或记录其中内容。
...
- 用户主动收藏的摘要片段只写入当前浏览器本机的 `chrome.storage.local`；收藏、搜索和删除不会发送新的网络请求。
- 字幕只发送到用户配置的 DeepSeek API，不经过项目自有服务器。
- 不展示逐字稿，不提供脱离圈选内容的开放式视频聊天、长期对话记录、个人备注、标签、导出、语音转文字或模型切换。
- 没有字幕的视频会明确提示暂不支持。
...
npm test
npm run build:release
unzip -t releases/skimline-0.3.2-extension.zip
