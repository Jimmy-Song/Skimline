...
1. 下载 `releases/skimline-0.2.0-extension.zip`。
2. 解压 ZIP，并进入其中的 `skimline-0.2.0` 目录。
...
在概览、观点和展开详情中圈选最多 200 个字，会出现“解释这段 / 复制”。解释会优先读取当前时间附近的字幕，再从整段字幕中检索相关内容；卡片区分通用定义与视频中的具体含义，并提供可跳转的字幕依据。每次解释最多继续追问 3 轮，关闭卡片、切换概念、语言或视频后立即结束，不写入本地缓存。
...
- 不展示逐字稿，不提供脱离圈选内容的开放式视频聊天、长期对话记录、笔记、导出、语音转文字或模型切换。
- 没有字幕的视频会明确提示暂不支持。
